"""Regression guards for maintenance ingestion call bindings."""
...
class MaintenanceBindingTests(unittest.TestCase):
    """验证维护版摄入调用的参数绑定。

    调用顺序：pytest/unittest 测试入口 -> MaintenanceBindingTests。
    """

    def test_faq_ingestion_passes_scenario_id_as_keyword(self) -> None:
        """验证 FAQ 摄入以关键字参数传入 scenario_id。

        调用顺序：pytest/unittest 测试入口 -> MaintenanceBindingTests.test_faq_ingestion_passes_scenario_id_as_keyword()。
        """
    ...
    def test_quality_report_passes_data_scope_as_keyword(self) -> None:
        ...
        调用顺序：pytest/unittest 测试入口 -> MaintenanceBindingTests.test_quality_report_passes_data_scope_as_keyword()。
        """
        call_block = _call_block(
