...
def prepare_retrieval(context: RAGQueryContext) -> RetrievalPreparation:
    ...
    query_variants = context.run_stage(
        ...
        lambda: generate_query_variants(
            ...
            domain="contract" if effective_source_filter == "contract" else None,
        ),
    )
...
def build_insufficient_context_answer(context: RAGQueryContext) -> str:
    ...
    if context.scenario.scenario_id == "tender_contract_risk":
...
def detect_and_apply_boundary_answer(context: RAGQueryContext) -> str | None:
    ...
    """
    # 合同专属入口已经由 contract dataset + source 双重限定，交付、付款等词是合同内部主题，
    # 不能再被通用分类边界误判为需要切换数据源。
    if context.source_filter == "contract" and str(context.data_scope.dataset_id).startswith("contract:"):
        context.retrieval_info["source_boundary"] = {
            "mismatched": False,
            "reason": "contract_dataset_isolation",
        }
        return None
