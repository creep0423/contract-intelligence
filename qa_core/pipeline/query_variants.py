...
CONTRACT_QUERY_EXPANSIONS: tuple[tuple[tuple[str, ...], str], ...] = (
    (("首付款", "首笔付款", "第一笔付款"), "预付款 首笔付款 合同签署后 支付比例 开票条件"),
    (("付款", "支付", "价款", "费用", "结算"), "付款节点 预付款 进度款 尾款 质保金 剩余款 支付条件 开票"),
    (("验收",), "验收条件 验收标准 验收期限 默认验收 整改"),
    (("违约", "违约金", "赔偿", "责任"), "违约责任 违约金 赔偿责任 责任上限 免责"),
    (("续约", "到期", "终止", "解除"), "自动续约 续约通知期 合同终止 单方解除"),
    (("知识产权", "著作权", "专利", "源代码"), "知识产权归属 著作权 专利 源代码 使用许可"),
)
...
def generate_query_variants(
    ...
    *,
    enabled: bool,
    allow_short_structured: bool = False,
    ...
    cleaned = query.strip()
    if domain == "contract" and cleaned:
        for keywords, expansion in CONTRACT_QUERY_EXPANSIONS:
