...
架构分层（自上而下）：
    ...
    scenarios/    → 合同运行配置解析（保留兼容抽象）
    storage/      → MySQL schema 初始化
    observability/→ LangSmith 可观测性适配

合同问答请求链路：
    浏览器 → WebSocket /api/contracts/{contract_id}/stream
    → api/contracts.py → ContractService.stream_qa()
    → 合同权限过滤、Milvus Hybrid Retrieval、证据校验与流式生成

底层 RAG 复用链路：
    application/QAService → pipeline/rag.py stream_query()
