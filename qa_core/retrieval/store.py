...
class MilvusHybridStore:
    ...
    def _similarity_search_with_score(self, query: str, *, k: int, expr: str) -> list[tuple[Document, float]]:
        ...
        try:
            return self.store.similarity_search_with_score(
                ...
                k=k,
                expr=expr,
                timeout=self.settings.milvus_query_timeout_seconds,
                **HYBRID_RANKER_KWARGS,
            )
        except MilvusException as exc:
            message = str(exc)
    ...
    def search_many(
        ...
        *,
        ...
        if rerank and hits:
            # 合同短问法（例如“首付款比例”）需要把确定性领域扩展一并交给 CrossEncoder；
            # 否则首轮虽已召回“预付款”条款，重排仍可能仅凭“首/付款”把“第一阶段付款”排在前面。
            # 普通知识库继续沿用原问题，避免改变既有检索语义。
            rerank_query = searched_queries[0]
            if (
                data_scope is not None
                and str(data_scope.dataset_id or "").startswith("contract:")
                and len(searched_queries) > 1
            ):
                rerank_query = " ".join(searched_queries)
            hits = self._rerank(rerank_query, hits)
        return RetrievalResult(
            hits=hits[:k],
