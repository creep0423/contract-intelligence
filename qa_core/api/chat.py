...
async def _send_stream_events(websocket: WebSocket, context: QueryServiceContext) -> bool:
    ...
    while True:
        ...
        if websocket.client_state != WebSocketState.CONNECTED:
            return False
        if context.targets_contract_namespace() and event.get("type") == "token":
            contract_answer = (contract_answer + str(event.get("token") or ""))[-4000:]
        if (
            context.targets_contract_namespace()
            and event.get("type") == "end"
            and "未发现明确约定" in contract_answer
        ):
            # 召回候选只是回答上下文。模型明确拒答时，不能把无关候选包装成事实依据。
            event = {**event, "sources": []}
...
def get_history(session_id: str):
    ...
    """
    if session_id.startswith("contract:"):
        raise_bad_request("合同会话历史必须通过合同权限接口读取")
...
def clear_history(session_id: str):
    ...
    """
    if session_id.startswith("contract:"):
        raise_bad_request("合同会话历史必须通过合同权限接口操作")
    ...
    try:
        while True:
            ...
            if not context.query:
                ...
                continue
            if context.targets_contract_namespace():
                await websocket.send_json({"type": "error", "error": "合同问答必须通过授权合同入口"})
                continue
...
async def debug_retrieval(request: RetrievalDebugRequest, _: None = Depends(enforce_http_rate_limit)):
    ...
    if context.targets_contract_namespace():
        raise_bad_request("合同检索必须通过授权合同入口")
...
def get_sources(scenario_id: str | None = None):
    ...
    """
    # 解析请求对应的业务场景配置
    scenario = resolve_scenario("tender_contract_risk")
    return {
