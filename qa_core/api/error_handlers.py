...
- 未预期异常统一记录请求位置和异常类型，但响应给前端时不暴露内部异常细节。
...
def register_api_exception_handlers(app: FastAPI) -> None:
    ...
    2. Exception -> 500 Internal Server Error：未预期异常记录日志，前端只收通用文案。
       原因：不暴露内部异常细节（如堆栈、SQL、文件路径）给前端或应用日志。
    ...
    async def contract_not_found_handler(_: Request, __: ContractNotFoundError) -> JSONResponse:
        return JSONResponse(status_code=404, content={"detail": "合同不存在或无权访问"})

    @app.exception_handler(ContractConflictError)
    async def contract_conflict_handler(_: Request, exc: ContractConflictError) -> JSONResponse:
    ...
    async def unexpected_error_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        同时使用合同流程自身的 contract_id/run_id/stage 日志关联排查。
