...
from qa_core.contracts.store import (
    CONTRACT_ANALYSES_TABLE,
    CONTRACT_DOCUMENTS_TABLE,
    CONTRACT_EVENTS_TABLE,
    CONTRACT_OBLIGATIONS_TABLE,
    CONTRACT_RISKS_TABLE,
    CONTRACTS_TABLE,
)
...
def bootstrap_mysql_schema() -> dict[str, object]:
    ...
    contracts_table = safe_sql_identifier(CONTRACTS_TABLE, label="Contracts table")
    contract_documents_table = safe_sql_identifier(CONTRACT_DOCUMENTS_TABLE, label="Contract documents table")
    contract_analyses_table = safe_sql_identifier(CONTRACT_ANALYSES_TABLE, label="Contract analyses table")
    contract_obligations_table = safe_sql_identifier(CONTRACT_OBLIGATIONS_TABLE, label="Contract obligations table")
    contract_risks_table = safe_sql_identifier(CONTRACT_RISKS_TABLE, label="Contract risks table")
    contract_events_table = safe_sql_identifier(CONTRACT_EVENTS_TABLE, label="Contract events table")
    ...
    statement_count = run_runtime_schema_sql(
        engine,
        {
            ...
            "CONTRACT_DOCUMENTS_TABLE": contract_documents_table,
            "CONTRACT_ANALYSES_TABLE": contract_analyses_table,
            "CONTRACT_OBLIGATIONS_TABLE": contract_obligations_table,
            "CONTRACT_RISKS_TABLE": contract_risks_table,
            "CONTRACT_EVENTS_TABLE": contract_events_table,
        },
    )
