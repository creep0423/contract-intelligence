...
class _MySqlStore:
    ...
    def engine(self):
        ...
        if self._engine is None:
            ...
            # SQLAlchemy 异常默认会把绑定参数写入 traceback；合同正文/证据可能位于参数中。
            # 隐藏参数仍保留 SQL 与异常类型，避免日志泄露合同数据和令牌。
            self._engine = create_engine(self.settings.mysql_sync_uri, pool_pre_ping=True, hide_parameters=True)
        return self._engine
